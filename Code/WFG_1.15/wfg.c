...
// this sorts points worsening in the last objective
{
	...
	{
		if
			BEATS(p.objectives[i], q.objectives[i])
			{
				return -1;
			}
		else if
			BEATS(q.objectives[i], p.objectives[i])
			{
				return 1;
			}
	}
	...
	for (int i = k; i >= 0; i--)
	{
		if
			BEATS(p.objectives[i], q.objectives[i])
			{
				...
				{
					if
						BEATS(q.objectives[j], p.objectives[j])
						{
							return 0;
						}
				}
			...
			BEATS(q.objectives[i], p.objectives[i])
			{
				...
				{
					if
						BEATS(p.objectives[j], q.objectives[j])
						{
							return 0;
						}
				}
...
// processes each front from the file
{
	...
	strcat(resultname, ".result.csv");
	int exist = fileexists(resultname);
	if (exist)
		printf("File %s exist", resultname);
		remove(resultname);
	
	FILE *fresult = fopen(resultname, "w");
	...
	for (int i = 0; i < f->nFronts; i++)
	{
		...
		n = f->fronts[i].n;
		safe = 0;
		double hvresult = hv(f->fronts[i]);
		printf("hv(%d) = %1.10f\n", i + 1, hvresult);
		fprintf(fresult, "%1.10f;", hvresult);

		getrusage(RUSAGE_SELF, &ru_after);
		tv1 = ru_before.ru_utime;
		tv2 = ru_after.ru_utime;
		double hvtime = tv2.tv_sec + tv2.tv_usec * 1e-6 - tv1.tv_sec - tv1.tv_usec * 1e-6;
		printf("Time: %f (s)\n", hvtime);
		fprintf(fresult, "%f\n", hvtime);
